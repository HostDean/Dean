...
public class BlazingElemental implements CombatStrategy {

	private static final Animation anim = new Animation(1818);

	private static final Graphic gfx1 = new Graphic(1618, 3, GraphicHeight.LOW);
	...
	public boolean customContainerAttack(Character entity, Character victim) {
		...
		if(Locations.goodDistance(uc.getPosition().copy(), victim.getPosition().copy(), 1) && Misc.getRandom(6) <= 4) {
			...
			uc.getCombatBuilder().setContainer(new CombatContainer(uc, victim, 1, 2, CombatType.MAGIC, true));
		} else if(Misc.getRandom(10) <= 7) {
			uc.performAnimation(anim);
			uc.setChargingAttack(true);
			uc.forceChat("I'm your daddy!");
			...
			uc.getCombatBuilder().setContainer(new CombatContainer(uc, victim, 1, 5, CombatType.RANGED, true));
			TaskManager.submit(new Task(1, uc, false) {
				int tick = 0;
				@Override
				public void execute() {
					if(tick == 0) {
						p.getPacketSender().sendGlobalGraphic(new Graphic(481), start);
						p.getPacketSender().sendGlobalGraphic(new Graphic(481), second);
						p.getPacketSender().sendGlobalGraphic(new Graphic(481), last);
					} else if(tick == 3) {
						for(Player t : list) {
							if(t == null)
								continue;
